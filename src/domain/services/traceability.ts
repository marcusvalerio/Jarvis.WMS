...
async function movementNodes(where: string, ...params: any[]): Promise<TraceNode[]> {
...
// ------------------------------------------------------------------ produto
export async function traceProduct(productId: string): Promise<TraceResult> {
  const p = await one<any>(`SELECT * FROM products WHERE id = ? OR sku = ?`, productId, productId);
  if (!p) {
    return { kind: "PRODUCT", id: productId, title: productId, subtitle: "", timeline: [], related: [], notFound: true };
  }
  const lots = await all<any>(`SELECT * FROM lots WHERE product_id = ?`, p.id);
  ...
  return {
    ...
    timeline: await movementNodes("m.product_id = ?", p.id),
    related,
  };
}

// ------------------------------------------------------------------ palete
export async function tracePallet(palletId: string): Promise<TraceResult> {
  const pl = await one<any>(
  ...
  const timeline: TraceNode[] = [
    ...
    ...await movementNodes("m.pallet_id = ?", palletId),
  ];
...
// ------------------------------------------------------------------ pedido
export async function traceOrder(orderId: string): Promise<TraceResult> {
  ...
  for (const pk of await all<any>(`SELECT * FROM picking_orders WHERE sales_order_id = ?`, orderId)) {
    if (pk.started_at) {
      timeline.push({
  ...
  for (const pa of await all<any>(`SELECT * FROM packing_orders WHERE sales_order_id = ?`, orderId)) {
    if (pa.completed_at) {
      timeline.push({
  ...
  for (const c of await all<any>(`SELECT * FROM shipping_checks WHERE sales_order_id = ?`, orderId)) {
    if (c.finished_at) {
      timeline.push({
  ...
  timeline.push(...await movementNodes("m.ref_id = ? AND m.ref_kind IN ('SALES_ORDER','SHIPMENT','PICKING')", orderId));
  if (o.shipped_at) {
    timeline.push({
...
// ------------------------------------------------------------------ recebimento
export async function traceInbound(inboundId: string): Promise<TraceResult> {
  ...
  for (const w of await all<any>(`SELECT * FROM weighings WHERE ref_kind = 'INBOUND_ORDER' AND ref_id = ?`, inboundId)) {
  ...
  for (const c of await all<any>(`SELECT * FROM receiving_checks WHERE inbound_order_id = ?`, inboundId)) {
    if (c.finished_at) {
  ...
  timeline.push(...await movementNodes("m.ref_kind = 'INBOUND_ORDER' AND m.ref_id = ?", inboundId));
  if (io.completed_at) timeline.push({ at: io.completed_at, stage: "Conclusao", tone: "success", title: "Recebimento concluido", detail: "" });
  timeline.sort((a, b) => a.at.localeCompare(b.at));

  const pallets = await all<any>(`SELECT id FROM pallets WHERE origin_ref = ?`, inboundId);
  return {
...
export async function traceDocument(docId: string): Promise<TraceResult> {
  const logs = await auditFor("invoice", docId);
  const invoice = await one<any>(`SELECT * FROM invoices WHERE id = ?`, docId);
  if (invoice) {
    ...
    const chain = invoice.inbound_order_id
      ? (await traceInbound(invoice.inbound_order_id)).timeline
      : invoice.sales_order_id ? (await traceOrder(invoice.sales_order_id)).timeline : [];
    return {
...
export async function traceAny(rawId: string): Promise<TraceResult> {
  const id = rawId.trim().toUpperCase();
  if (id.startsWith("PLT-")) return await tracePallet(id);
  if (id.startsWith("PED-")) return await traceOrder(id);
  if (id.startsWith("OR-")) return await traceInbound(id);
  if (id.startsWith("NFS-")) return await traceDocument(id);
  const product = await one<any>(`SELECT id FROM products WHERE id = ? OR sku = ?`, id, id);
  if (product) return await traceProduct(product.id);
