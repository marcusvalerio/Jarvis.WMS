...
export function createCount(params: {
  ...
  return tx(() => {
    const at = nowIso();
    const id = nextId(PREFIX.INVENTORY_COUNT);
...
export function listCounts(filter: { status?: string } = {}) {
...
export function startCount(id: string, operatorId: string) {
  return tx(() => {
    const at = nowIso();
    run(
...
export function currentCountItem(countId: string) {
  return one<any>(
...
export function countItem(params: {
  ...
  return tx(() => {
    const at = nowIso();
    const item = one<any>(`SELECT * FROM inventory_count_items WHERE id = ?`, params.itemId);
    ...
    if (divergence !== 0) {
      const loc = one<any>(`SELECT code FROM locations WHERE id = ?`, item.location_id);
      const prod = one<any>(`SELECT sku FROM products WHERE id = ?`, item.product_id);
      openIncident({
...
export function closeCount(params: { countId: string; operatorId: string; applyAdjustments: boolean }) {
  return tx(() => {
    const at = nowIso();
    ...
    if (params.applyAdjustments) {
      for (const it of items) {
        ...
        run(`UPDATE inventory_count_items SET status = 'ADJUSTED', adjusted = 1 WHERE id = ?`, it.id);
        adjusted++;
      }
    }

    const accuracy = accuracyOf(params.countId);
    run(
 ...
 */
export function accuracyOf(countId: string): number {
  const r = one<any>(
...
export function globalAccuracy(): { accuracy: number; counted: number; divergent: number } {
  const r = one<any>(
